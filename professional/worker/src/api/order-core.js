...
function validateItems(rawItems,relationMap){
  return rawItems.map((raw,index)=>{
    ...
    const unitsPerOrderUnit=number(raw.unitsPerOrderUnit,{min:0.001,max:100000,fallback:Number(relation.units_per_order_unit||1)});
    const orderUnit=optionalText(raw.orderUnit||relation.order_unit||'UNIDAD',{max:60}).toUpperCase()||'UNIDAD';
    return {
...
export async function createOrderBatchV2(request,env,actor,ctx){
  ...
  const relationMap=await loadRelations(env,actor,center.id,rawItems);
  const items=validateItems(rawItems,relationMap);
  ...
  for(const group of groups.values()){
    ...
    summaries.push({id:orderId,folio,status,supplierId:group.supplierId,supplierName:group.supplierName,itemCount:group.items.length,locationId:location.id,costCenterId:center.id,pdfPending:true});
    statements.push(env.DB.prepare(`
      INSERT INTO orders(id,org_id,location_id,supplier_id,folio,status,requested_by,delivery_date,notes,currency,gross_total,revision,created_at,updated_at)
      VALUES(?,?,?,?,?,?,?,?,?,'CLP',?,1,?,?)
    `).bind(orderId,actor.orgId,location.id,group.supplierId,folio,status,actor.userId,body.deliveryDate||null,optionalText(body.notes,{max:2000}),grossTotal,timestamp,timestamp));
    ...
    group.items.forEach((item,sortOrder)=>{
      statements.push(env.DB.prepare(`
        INSERT INTO order_items(id,order_id,supplier_product_id,product_id,description_snapshot,quantity_ordered,order_unit_snapshot,units_per_order_unit,expected_gross_unit_price,expected_gross_total,sort_order,created_at,updated_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)
      `).bind(uuid(),orderId,item.supplierProductId,item.productId,item.description,item.quantity,item.orderUnit,item.unitsPerOrderUnit,item.expectedGrossUnitPrice,Math.round(item.quantity*item.unitsPerOrderUnit*item.expectedGrossUnitPrice),sortOrder,timestamp,timestamp));
  ...
  statements.push(env.DB.prepare(`
    INSERT INTO usage_counters(org_id,month_key,metric,quantity,updated_at) VALUES(?,?,?,?,?)
    ON CONFLICT(org_id,month_key,metric) DO UPDATE SET quantity=quantity+excluded.quantity,updated_at=excluded.updated_at
  `).bind(actor.orgId,monthKey(),'orders_created',summaries.length,timestamp));
  ...
  await writeAudit(env,actor,request,'order.batch_v2_create','order_batch',batchId,{locationId:location.id,costCenterId:center.id,orders:summaries.map(order=>order.id),status});

  const pdfTask=Promise.allSettled(summaries.map(async summary=>{
    const order=await getOrder(env,actor,summary.id);
...
export async function ensureOrderPdf(request,env,actor,orderId){
...
export async function listOrderInvoices(env,actor,orderId){
  ...
  const result=await env.DB.prepare(`
    SELECT i.id,i.invoice_number,i.invoice_date,i.status,i.gross_total,i.net_total,i.tax_total,i.created_at,
    ...
    WHERE l.org_id=? AND l.order_id=?
    ORDER BY i.invoice_date DESC,i.created_at DESC
  `).bind(actor.orgId,orderId).all();
  return rows(result).map(invoice=>({id:invoice.id,invoiceNumber:invoice.invoice_number,invoiceDate:invoice.invoice_date,status:invoice.status,grossTotal:Number(invoice.gross_total||0),netTotal:Number(invoice.net_total||0),taxTotal:Number(invoice.tax_total||0),pdfKey:invoice.pdf_key||'',pdfName:invoice.pdf_name||'',createdAt:invoice.created_at}));
}
