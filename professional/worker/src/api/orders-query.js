...
function filters(actor,url){
  ...
  const supplier=String(url.searchParams.get('supplier')||''),location=String(url.searchParams.get('location')||''),center=String(url.searchParams.get('center')||''),status=String(url.searchParams.get('status')||''),invoice=String(url.searchParams.get('invoice')||''),reception=String(url.searchParams.get('reception')||''),from=String(url.searchParams.get('from')||''),to=String(url.searchParams.get('to')||''),q=String(url.searchParams.get('q')||'').trim();
...
async function metadata(env,actor,scope){
  const locationWhere=scope?` AND id IN (${placeholders(scope.length?scope:['__none__'])})`:'';
  const centerWhere=scope?` AND location_id IN (${placeholders(scope.length?scope:['__none__'])})`:'';
  const locationParams=[actor.orgId,...(scope||[])],centerParams=[actor.orgId,...(scope||[])];
  const[suppliersResult,locationsResult,centersResult,categoriesResult]=await Promise.all([
  ...
  return{suppliers:rows(suppliersResult),locations:rows(locationsResult),costCenters:rows(centersResult),categories:rows(categoriesResult),brands:[]};
}

export async function listOrdersCanonical(env,actor,url){
  const limit=clamp(url.searchParams.get('limit')||60,20,150),{conditions,params,view,scope}=filters(actor,url),result=await env.DB.prepare(`SELECT o.id,o.folio,o.status,o.delivery_date,o.created_at,o.updated_at,o.sent_at,o.emitted_at,o.gross_total,o.supplier_id,o.location_id,
