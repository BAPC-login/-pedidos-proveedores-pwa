...
import {listProducts as listProductsBase} from './catalog-scoped.js';
...
async function nextSortOrder(env,orgId,costCenterId){
  const row=await env.DB.prepare(`SELECT COALESCE(MAX(c.sort_order),-1) AS value FROM categories c JOIN category_cost_centers ccc ON ccc.category_id=c.id WHERE c.org_id=? AND ccc.cost_center_id=? AND c.source='user'`).bind(orgId,costCenterId).first();
...
export async function listCategoriesV14(env,actor,url){
  ...
  const result=await env.DB.prepare(`
    SELECT c.id,c.name,c.sort_order,c.active,c.source,ccc.cost_center_id,
      ...
      (SELECT COUNT(*) FROM product_center_categories pcc WHERE pcc.org_id=c.org_id AND pcc.category_id=c.id AND pcc.cost_center_id=ccc.cost_center_id) AS product_count
    FROM categories c
  ...
  return rows(result).filter(item=>locationAllowed(actor,item.location_id)).map(item=>({id:item.id,name:item.name,sortOrder:Number(item.sort_order||0),active:Boolean(item.active),source:'user',costCenterId:item.cost_center_id,costCenterName:item.cost_center_name,locationId:item.location_id,locationName:item.location_name,productCount:Number(item.product_count||0)}));
}
...
export async function createCategoryV14(request,env,actor){
  ...
  const timestamp=nowIso(),sortOrder=await nextSortOrder(env,actor.orgId,center.id);let id=existing?.id||uuid(),reused=false;
  ...
  if(existing){
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(`
        INSERT OR REPLACE INTO product_center_categories(product_id,org_id,cost_center_id,category_id,created_at,updated_at)
        SELECT p.id,p.org_id,?,p.category_id,?,?
        FROM products p JOIN product_cost_centers pcc ON pcc.product_id=p.id AND pcc.org_id=p.org_id AND pcc.cost_center_id=?
        WHERE p.org_id=? AND p.category_id=? AND p.active=1
      `).bind(center.id,timestamp,timestamp,center.id,actor.orgId,id)
    ]);
  }else{
    await env.DB.batch([
...
export async function updateCategoryV14(request,env,actor,categoryId){
  assertMinimumRole(actor.role,ROLES.PURCHASER);
  const current=await env.DB.prepare(`SELECT c.id,c.name,ccc.cost_center_id FROM categories c JOIN category_cost_centers ccc ON ccc.category_id=c.id WHERE c.id=? AND c.org_id=? AND c.source='user'`).bind(categoryId,actor.orgId).first();
...
export async function setProductCenterCategoryV14(request,env,actor,productId){
  ...
  const product=await env.DB.prepare(`SELECT p.id FROM products p JOIN product_cost_centers pcc ON pcc.product_id=p.id AND pcc.org_id=p.org_id WHERE p.id=? AND p.org_id=? AND pcc.cost_center_id=? AND p.active=1`).bind(productId,actor.orgId,center.id).first();
  if(!product)throw new HttpError(400,'El producto no está asignado a este centro de costo','product_outside_cost_center');
  if(categoryId){
    const category=await env.DB.prepare(`SELECT c.id FROM categories c JOIN category_cost_centers ccc ON ccc.category_id=c.id AND ccc.org_id=c.org_id WHERE c.id=? AND c.org_id=? AND c.source='user' AND c.active=1 AND ccc.cost_center_id=?`).bind(categoryId,actor.orgId,center.id).first();
    if(!category)throw new HttpError(400,'La categoría no pertenece a este centro de costo','category_center_mismatch');
    await env.DB.prepare(`INSERT INTO product_center_categories(product_id,org_id,cost_center_id,category_id,created_at,updated_at) VALUES(?,?,?,?,?,?) ON CONFLICT(product_id,cost_center_id) DO UPDATE SET category_id=excluded.category_id,updated_at=excluded.updated_at`).bind(productId,actor.orgId,center.id,categoryId,nowIso(),nowIso()).run();
  }else await env.DB.prepare('DELETE FROM product_center_categories WHERE product_id=? AND org_id=? AND cost_center_id=?').bind(productId,actor.orgId,center.id).run();
  const primary=await env.DB.prepare(`SELECT pcc.category_id FROM product_center_categories pcc JOIN product_cost_centers assigned ON assigned.org_id=pcc.org_id AND assigned.product_id=pcc.product_id AND assigned.cost_center_id=pcc.cost_center_id WHERE pcc.product_id=? AND pcc.org_id=? ORDER BY pcc.updated_at DESC LIMIT 1`).bind(productId,actor.orgId).first();
...
export async function listProductsV14(env,actor,url){
  ...
  const result=await env.DB.prepare(`
    ...
    JOIN product_cost_centers assigned ON assigned.org_id=pcc.org_id AND assigned.product_id=pcc.product_id AND assigned.cost_center_id=pcc.cost_center_id
  ...
  const map=new Map();
  for(const row of rows(result)){if(!locationAllowed(actor,row.location_id))continue;if(!map.has(row.product_id))map.set(row.product_id,[]);map.get(row.product_id).push({costCenterId:row.cost_center_id,costCenterName:row.cost_center_name,categoryId:row.category_id,categoryName:row.category_name})}
  return products.map(product=>{const centerCategories=map.get(product.id)||[],primary=centerCategories[0];return {...product,centerCategories,categoryId:primary?.categoryId||null,categoryName:primary?.categoryName||null}});
}
