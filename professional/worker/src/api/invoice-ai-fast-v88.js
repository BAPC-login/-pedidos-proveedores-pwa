...
export async function analyzeInvoiceFastV88(env,file,context={}){
  ...
  try{
    const first=await callModelResilient(env,{model:primary,mime,data,prompt:basePrompt(file.name,nature),thinkingLevel:'minimal',timeoutMs:PRIMARY_TIMEOUT_MS},{attempts,stage:'primary',startedAt,maxRetries:2}),math=candidateMath(first.raw,nature);
    if(math.ok)return{ok:true,model:primary,usage:first.usage,invoice:first.raw,verification:{stage:'deterministic-precheck',...math},attempts,resilienceV91:true};
    attempts.push({model:primary,stage:'primary-validation',reason:math.reason});
    const repaired=await callModelResilient(env,{model:primary,mime,data,prompt:repairPrompt(file.name,math,nature),thinkingLevel:'medium',timeoutMs:REPAIR_TIMEOUT_MS},{attempts,stage:'repair',startedAt,maxRetries:1}),repairedMath=candidateMath(repaired.raw,nature);
    ...
    return{ok:true,model:primary,usage:repaired.usage,invoice:repaired.raw,verification:{stage:'targeted-repair',...repairedMath},attempts,resilienceV91:true};
  }catch(error){
