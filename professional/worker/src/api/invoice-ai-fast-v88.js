...
const extractionInstruction=`Eres el lector documental de Nuvasto. Tu trabajo es TRANSCRIBIR evidencia visible de facturas, guías, boletas y notas de crédito; no eres el motor contable ni debes inventar cálculos.
...
11. Devuelve solo JSON conforme al schema. La aplicación hará cotejo de catálogo y validación matemática fuera de la IA.`;
...
function candidateMath(raw){const items=Array.isArray(raw?.items)?raw.items:[],totals=raw?.totals||{},documentTotal=num(totals.total);if(!items.length||!documentTotal)return{ok:false,reason:'missing-lines-or-total'};const lineValues=items.map(item=>num(item.lineTotal||item.grossLineTotal||item.netLineTotal)).filter(value=>value>0);if(lineValues.length!==items.filter(item=>!item.isFree).length)return{ok:true,reason:'partial-line-values'};const sum=lineValues.reduce((a,b)=>a+b,0),known=[num(totals.freight),num(totals.vat),num(totals.additionalTax),num(totals.other)],targets=new Set([documentTotal,num(totals.net)]);for(let mask=0;mask<(1<<known.length);mask++){let outside=0;for(let i=0;i<known.length;i++)if(mask&(1<<i))outside+=known[i];if(documentTotal-outside>0)targets.add(documentTotal-outside)}const ok=[...targets].some(target=>target>0&&close(sum,target,2));return{ok,reason:ok?'sum-reconciles':'line-sum-does-not-reconcile',sum,documentTotal,targets:[...targets].filter(Boolean)}}
function parseCandidate(payload){const text=payload?.candidates?.[0]?.content?.parts?.map(part=>part.text||'').join('')||'';if(!text)throw new Error('Gemini no devolvió contenido estructurado');return JSON.parse(text)}
...
function basePrompt(fileName){return`Lee el documento “${String(fileName||'documento').slice(0,180)}”. Extrae primero cabecera y totales, luego cada fila comercial. Respeta literalmente los encabezados de columnas de cantidad y valores. Si una columna dice VALOR TOTAL, transcríbela como lineTotal sin decidir si es neta o bruta. No cotejes productos ni hagas cálculos contables.`}
function repairPrompt(fileName,math){return`Segunda lectura dirigida del documento “${String(fileName||'documento').slice(0,180)}”. La primera extracción no pudo cerrar matemáticamente las columnas de importes (suma=${num(math.sum)}, total documento=${num(math.documentTotal)}). Relee SOLO con especial cuidado: cantidad, encabezados de precio/valor por fila, valores de cada fila y bloque de neto/IVA/impuestos/flete/total. No fuerces una suma: si un valor no se ve, deja 0 y marca incertidumbre. Devuelve nuevamente el documento completo según el schema.`}

export async function analyzeInvoiceFastV88(env,file){
  if(!env.GEMINI_API_KEY)throw Object.assign(new Error('Gemini no está configurado'),{status:503,code:'missing_api_key'});
  const mime=file.type||(/\.pdf$/i.test(file.name||'')?'application/pdf':'image/jpeg'),data=toBase64(await file.arrayBuffer()),primary=env.GEMINI_MODEL||DEFAULT_MODEL,attempts=[];
  try{
    const first=await callModel(env,{model:primary,mime,data,prompt:basePrompt(file.name),thinkingLevel:'minimal',timeoutMs:PRIMARY_TIMEOUT_MS}),math=candidateMath(first.raw);
    ...
    const repaired=await callModel(env,{model:primary,mime,data,prompt:repairPrompt(file.name,math),thinkingLevel:'medium',timeoutMs:REPAIR_TIMEOUT_MS}),repairedMath=candidateMath(repaired.raw);
    if(!repairedMath.ok)throw Object.assign(new Error('La lectura no pudo verificarse contra los totales visibles del documento'),{code:'invoice_math_unverified',details:{math:repairedMath}});
