...
function normalizeSql(moduleValue,label){const raw=typeof moduleValue==='string'?moduleValue:moduleValue?.default;if(typeof raw!=='string'||!raw.trim())throw new Error(`SQL module ${label} did not resolve to text`);return raw.replace(/^\s*PRAGMA\s+foreign_keys\s*=\s*ON\s*;\s*/gim,'').trim()}
function prepareSchemaStatements(db,sql,label){const statements=sql.split(';').map(statement=>statement.trim()).filter(Boolean).map((statement,index)=>{try{return db.prepare(statement)}catch(error){throw new Error(`Could not prepare ${label} statement ${index+1}: ${error?.message||error}`)}});if(!statements.length)throw new Error(`Schema ${label} has no executable statements`);return statements}
...
async function ensureV13(db){
  ...
  await db.batch([
    ...
    db.prepare(`UPDATE categories SET source='system' WHERE id LIKE 'seed-category-%' OR id LIKE 'legacy-category-%'`),
...
export async function ensureSchema(env){
  ...
  initializationPromise=(async()=>{
    ...
    await env.DB.prepare(`UPDATE categories SET source='system' WHERE id LIKE 'seed-category-%' OR id LIKE 'legacy-category-%'`).run();
    return {initialized:true,seeded,ownerPasswordMigrated,...v13,legacyCatalog,version:SCHEMA_VERSION,statements:identityStatements+procurementStatements+invoiceStatements+platformStatements+fileChunkStatements+costCenterStatements};
