...
import {ensureOrderPdfV18} from './api/order-pdf-v18.js';
...
export default{async fetch(request,env,ctx){
  ...
  if(!scopedHealth&&!scopedDashboard&&!scopedProducts&&!scopedSuppliers&&!scopedCategories&&!scopedWorkbook&&!scopedReadiness&&!scopedPdf)return platformWorker.fetch(request,env,ctx);
  try{
    ...
    else if(orderPdfParams&&method==='POST')payload={document:await ensureOrderPdfV18(request,env,actor,orderPdfParams.id)};
    ...
    else throw new Error('Ruta v18 no implementada');
