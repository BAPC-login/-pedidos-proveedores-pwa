export function matchProcurementDomain(path,method){
  ...
  if(method==='POST'&&path==='/api/order-batches/v2')return{implementation:'procurement',layer:'orders'};
  if(method==='POST'&&/^\/api\/order-batches\/[^/]+\/emit$/.test(path))return{implementation:'procurement',layer:'emission'};
  if(method==='POST'&&/^\/api\/orders\/[^/]+\/receptions$/.test(path))return{implementation:'procurement',layer:'reception'};
