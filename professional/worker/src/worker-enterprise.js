...
import {applyInvoicePaymentMetadata,createPaymentMethodCanonical,listChequeScheduleCanonical,listPaymentMethodsCanonical,listPaymentsCanonical,updatePaymentCanonical,updatePaymentMethodCanonical} from './api/payments.js';
import {
  addOrderCommentV41,
...
export default{async fetch(request,env,ctx){const url=new URL(request.url),method=request.method.toUpperCase(),path=url.pathname,
  ...
  try{
    ...
    if(method==='POST'&&path==='/api/invoices'){
      ...
      if(body.payment){try{paymentResult=await applyInvoicePaymentMetadata(env,actor,invoice.id,body.payment,request)}catch(error){throw error}}
    ...
    if(method==='GET'&&path==='/api/finance/cheques'){const actor=await auth(request,env);return decorate(ok({cheques:await listChequeScheduleCanonical(env,actor,url)},request,env),request,env)}
