...
function addHeaders(response,request,env){
  ...
  headers.set('X-Nuvasto-Version','27');
...
async function callInvoiceAi(env,{file,orderFile,context}){
  ...
  for(let attempt=1;attempt<=AI_ATTEMPTS;attempt++){
    ...
    try{
      ...
      const response=await fetch(`${endpoint}/v1/invoices/analyze`,{method:'POST',signal:controller.signal,headers:{'X-Pedidos-Client':'professional-v27'},body:upstream});
...
async function analyzeInvoiceV27(request,env,actor){
  ...
  context={...context,organizationId:actor.orgId,requestedBy:actor.userId,normalizationVersion:26,flowVersion:27};
  const sourceFile=await storeFile(env,actor,file,{purpose:'invoice-source',entityType:'invoice-analysis',entityId:crypto.randomUUID(),documentKind:'invoice_original_pending',metadata:{providerName:String(context.providerName||''),folio:String(context.folio||''),locationId:String(context.locationId||''),flowVersion:27}});
  ...
  await writeAudit(env,actor,request,'invoice.analyze','invoice','',{model:normalized.model||raw.model||'',fileName:file.name,sourceFileId:sourceFile.id,normalizationVersion:26,flowVersion:27,folio:String(context.folio||''),attempts:AI_ATTEMPTS,attemptTimeoutMs:AI_TIMEOUT_MS});
  return normalized;
}
...
async function health(request,env,ctx){
  ...
  return addHeaders(ok({...payload,version:'2.0.0-alpha.27',r2Configured:Boolean(env.FILES),r2Required:String(env.REQUIRE_R2||'').toLowerCase()==='true',r2Bucket:'nuvasto-files',invoiceNormalizationVersion:26,invoiceFlowVersion:27,invoiceRetryAttempts:AI_ATTEMPTS,invoiceAttemptTimeoutMs:AI_TIMEOUT_MS,keyboardNavigationVersion:27},request,env),request,env);
}
...
export default{async fetch(request,env,ctx){
  ...
  try{
    ...
    if(method==='POST'&&url.pathname==='/api/invoices/analyze'){
      const actor=await authenticate(request,env);return addHeaders(ok({analysis:await analyzeInvoiceV27(request,env,actor)},request,env),request,env);
    }
