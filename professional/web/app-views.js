...
async function renderDashboard(){
  ...
  $('#mainContent').innerHTML=`
    ...
    <section class="metrics-grid">
      ${metric('Pedidos 30 días',m.orders30d,'Órdenes creadas')}${metric('Pendientes',m.pendingOrders,'Requieren gestión')}${metric('Compras 30 días',money(m.spend30d),'Facturas registradas')}${metric('Proveedores',m.suppliers,'Activos')}${metric('Productos',m.products,'En catálogo')}${metric('Diferencias',m.openIssues,'Sin resolver')}${metric('Documentos',m.archivedDocuments||0,'Históricos archivados')}
    </section>
...
async function renderInvoices(){
  ...
  $('#invoiceSearch').addEventListener('input',()=>{const q=$('#invoiceSearch').value.toLowerCase();$('#invoiceContainer').innerHTML=invoiceTable(state.cache.invoices.filter(i=>`${i.invoiceNumber} ${i.supplierName} ${(i.locationNames||[]).join(' ')}`.toLowerCase().includes(q)));bindStoredDocuments()});bindDynamic();bindStoredDocuments();
}
