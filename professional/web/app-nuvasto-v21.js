...
let initialized=false,brandSynced=false,scheduled=false;
...
function applyMetadata(){
  document.title='Nuvasto';
  document.querySelector('meta[name=description]')?.setAttribute('content','Nuvasto: compras, abastecimiento, proveedores, recepción y facturas con trazabilidad.');
  document.querySelector('meta[name=theme-color]')?.setAttribute('content',BRAND.navy);
  document.documentElement.dataset.productBrand='nuvasto';
}
function enhanceBrandWorkspace(root=document){
  const cards=[...root.querySelectorAll?.('.v20-card')||[]];
  const card=cards.find(item=>item.textContent.includes('Marca del producto'));
  if(card&&!card.querySelector('.nuvasto-brand-panel')){
    card.querySelector('.metric')?.remove();card.querySelector('small')?.remove();
    card.querySelector('header')?.insertAdjacentHTML('afterend',`<div class="nuvasto-brand-panel"><img src="./nuvasto-mark.svg" alt="Isotipo Nuvasto"><div><h3>Nuvasto</h3><p>${BRAND.tagline}<br>Identidad seleccionada y aplicada en la PWA, documentos y comunicaciones.</p></div></div>`)
  }
}
...
function schedule(root=document){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>enhance(root))}
async function syncWorkspace(){
  if(brandSynced||!state.token||!['owner','admin'].includes(state.me?.user?.role))return;
  brandSynced=true;
  try{const payload=await api('/api/professional/brand',{fresh:true}),brand=payload.brand||payload;if(brand.productName!=='Nuvasto'||brand.status==='exploring')await api('/api/professional/brand',{method:'PUT',json:{productName:'Nuvasto',tagline:BRAND.tagline,status:'selected',palette:{navy:BRAND.navy,primary:BRAND.primary,secondary:BRAND.secondary,accent:BRAND.accent,cloud:'#F4F7FB'},candidates:[{name:'Nuvasto',tagline:BRAND.tagline,status:'selected'}]}})}catch(error){console.warn('nuvasto_brand_sync_failed',error)}
}
export function initializeNuvastoV21(){
  if(initialized)return;initialized=true;applyMetadata();schedule(document);
  new MutationObserver(records=>{for(const record of records)for(const node of record.addedNodes)if(node.nodeType===Node.ELEMENT_NODE)schedule(node)}).observe(document.body,{subtree:true,childList:true});
  window.addEventListener('pedidos:view-rendered',()=>{schedule(document);syncWorkspace()});
