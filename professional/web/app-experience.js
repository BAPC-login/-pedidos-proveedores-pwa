...
function injectNavigation(){
  ...
  if(side&&!side.querySelector('[data-experience-view="receiving"]')){
    const orders=side.querySelector('[data-view="orders"]');orders?.insertAdjacentHTML('afterend','<button class="nav-item" data-experience-view="receiving"><span class="nav-icon">⇣</span><span>Vigentes</span><b class="nav-count" id="receivingCount">0</b></button>');
  ...
  const bottom=$('.bottom-nav');if(bottom&&!bottom.dataset.experience){bottom.dataset.experience='1';bottom.innerHTML='<button class="bottom-item active" data-view="dashboard"><span>⌂</span><small>Inicio</small></button><button class="bottom-item" data-view="orders"><span>▤</span><small>Pedidos</small></button><button class="bottom-create" id="mobileCreate" type="button" aria-label="Nuevo pedido">＋</button><button class="bottom-item" data-experience-view="receiving"><span>⇣</span><small>Vigentes</small></button><button class="bottom-item" data-view="settings"><span>☰</span><small>Más</small></button>'}
}
