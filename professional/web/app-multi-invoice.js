...
function injectStyles(){
  ...
  const style=document.createElement('style');style.id='nuvastoMultiInvoiceV38Styles';style.textContent=`
    ...
    .v38-price-breakdown{grid-column:1/-1;display:grid;gap:4px;padding:10px 12px;border:1px solid color-mix(in srgb,var(--success) 22%,var(--line));border-radius:12px;background:color-mix(in srgb,var(--success) 5%,var(--card))}.v38-price-breakdown strong{font-size:9px}.v38-price-breakdown p{margin:0;color:var(--muted);font-size:8px;line-height:1.45}.v38-price-breakdown.review{border-color:color-mix(in srgb,var(--warning) 30%,var(--line));background:color-mix(in srgb,var(--warning) 5%,var(--card))}
...
function lineMoneyField(value,disabled=false){const amount=Math.max(0,integer(value));return `<label class="field"><span>Total final de la línea</span><input class="v38-money-input" type="text" inputmode="numeric" autocomplete="off" data-v38-line-money value="${esc(money(amount))}" ${disabled?'disabled':''}><input type="hidden" name="grossLineTotal" value="${amount}"></label>`}
...
async function openReview(analysis,item){
  ...
  openModal({eyebrow:`DOCUMENTO ${position+1} DE ${total}`,title:`Revisar ${kindLabel(item.kind).toLowerCase()}`,subtitle:'Confirma este documento de manera independiente antes de continuar con el siguiente.',size:'large',closeOnSuccess:false,body:reviewBody(analysis,lines,queue.order,item,position,total),submitLabel:item.kind==='free'?'Guardar documento sin cargo':'Guardar factura y continuar',onSubmit:async form=>{
    ...
    const reviewed=rows.map((row,index)=>{const original=lines[index],packageQty=Number(row.querySelector('[name=packageQty]').value||0),packSize=Number(row.querySelector('[name=packSize]').value||1),units=packageQty*packSize,isFree=item.kind==='free'||row.querySelector('[name=isFree]').checked,grossLineTotal=isFree?0:Number(row.querySelector('[name=grossLineTotal]').value||0),printedFinalUnitPrice=isFree?0:decimal(row.dataset.printedFinalUnit||0),excluded=rejected.has(index);return{...original,productId:row.querySelector('[name=productId]').value,sourceDescription:lineSource(original,index),packageQty,invoiceQuantity:packageQty,packSize,units,totalUnits:units,grossLineTotal,grossUnitPrice:isFree?0:(printedFinalUnitPrice>0?printedFinalUnitPrice:(units?Math.round(grossLineTotal/units):0)),printedFinalUnitPrice,finalUnitPrice:printedFinalUnitPrice||original.finalUnitPrice||0,priceSource:printedFinalUnitPrice>0?'printed-final-unit':original.priceSource,isFree,freeReason:isFree?(row.querySelector('[name=freeReason]').value||'Documento sin cargo'):'',policyAction:excluded?'reject':original.policyAction,rejectedByPolicy:excluded||original.rejectedByPolicy,policyReason:excluded?'Producto no solicitado':original.policyReason}});
...
async function processCurrent(){
  ...
  try{
    ...
    upload.append('file',item.file,item.file.name);upload.append('context',JSON.stringify({orderId:queue.orderId,supplierId:queue.supplierId,providerName:queue.order.supplierName,folio:queue.order.folio,products,aliases:queue.aliases.map(alias=>({productId:alias.productId,alias:alias.alias,confidence:alias.confidence,usageCount:alias.usageCount})),locationId:queue.locationId,fileName:item.file.name,documentNature:item.kind,batchPosition:position+1,batchTotal:total}));
...
async function openMultiDocuments(options={}){
  ...
  try{
    ...
    openModal({eyebrow:'ETAPA 2 · DOCUMENTOS',title:'Adjuntar documentos al pedido',subtitle:'Selecciona hasta 5 archivos. Cada uno se leerá, revisará y guardará de forma independiente.',size:'large',closeOnSuccess:false,body:`<div class="v30-field-grid">${context}<label class="field full"><span>Facturas y documentos sin cargo</span><div class="v38-upload-copy"><strong>Seleccionar uno o varios documentos</strong><small>PDF o fotografías completas, nítidas y tomadas de frente.</small></div><input id="v38Files" name="files" type="file" accept="application/pdf,image/*" multiple required></label></div><div class="v38-selected-list" id="v38SelectedDocuments"><div class="v38-selected-empty">Selecciona entre 1 y 5 facturas o documentos sin cargo.</div></div><section class="v38-batch-note"><strong>Un pedido puede tener varias facturas</strong><p>Por ejemplo: bebidas, destilados, aguas y un documento sin cargo. Cada archivo conservará su número, fecha, total, líneas y original en R2.</p></section>`,submitLabel:'Procesar documentos',onSubmit:async form=>{
      ...
      if(!activeOrder(order))throw new Error('El pedido ya no admite nuevos documentos.');const kinds=files.map((file,index)=>({file,kind:$(`[data-v38-kind="${index}"]`)?.value==='free'?'free':'invoice'})),aliases=(await api(`/api/aliases?supplierId=${encodeURIComponent(supplierId)}`,{fresh:true,timeout:20000}).catch(()=>({aliases:[]}))).aliases||[];
      queue={id:crypto.randomUUID(),active:true,processing:false,index:0,items:kinds,saved:[],skipped:[],order,orderId,supplierId,locationId,aliases,returnToHistory:options.returnToHistory!==false};window.NuvastoInvoiceQueue={active:true,id:queue.id,total:kinds.length,orderId};revokePreviews();await processCurrent();
    }});
...
async function resolveOrderId(trigger){
  ...
  const candidates=[$('#modalEyebrow')?.textContent,$('#modalTitle')?.textContent].map(value=>String(value||'').trim()).filter(Boolean);
  for(const candidate of candidates){const order=state.cache.orders.find(item=>String(item.folio||'').trim()===candidate);if(order?.id)return order.id}
  return'';
}
