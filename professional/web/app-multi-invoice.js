...
function lineMoneyField(value,disabled=false){const amount=Math.max(0,integer(value));return `<label class="field"><span>Bruto final calculado</span><input class="v38-money-input" type="text" inputmode="numeric" autocomplete="off" data-v38-line-money value="${esc(money(amount))}" ${disabled?'disabled':''}><input type="hidden" name="grossLineTotal" value="${amount}"></label>`}
