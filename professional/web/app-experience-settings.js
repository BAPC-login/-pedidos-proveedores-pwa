  ...
  try{
    ...
    $('#mainContent').innerHTML=`<div id="settingsExperienceHub">
      ...
      <section class="settings-section" id="settingsOperation"><div class="settings-section-head"><div><h3>Operación</h3><p>Locales, centros de costo y reglas comunes. El recorrido Bodega → Categoría → Producto permanece en Catálogo.</p></div><div class="view-actions"><button class="btn" data-action="new-location">＋ Local</button><button class="btn primary" data-action="new-cost-center">＋ Centro</button></div></div><div class="settings-menu-grid">
        ${card('⌂','Datos de locales','Razón social, RUT, dirección y contacto específico de cada local.','data-settings-panel="locations"','Editar locales',`<div class="settings-inline-meta"><span>${countLabel(state.cache.locations.length,'local','locales')}</span></div>`)}
        ${card('▣','Identidad de locales','Carga o reemplaza el logo propio de cada local sin alterar el perfil de empresa.','data-location-identity','Editar logos',`<div class="settings-inline-meta"><span>${countLabel(state.cache.locations.length,'local','locales')}</span></div>`)}
