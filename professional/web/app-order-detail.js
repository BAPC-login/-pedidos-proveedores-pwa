...
async function openReception(order){
  openModal({
    ...
    onSubmit:async form=>{
      const items=$$('[data-reception-line]').map(row=>({
        orderItemId:row.dataset.receptionLine,
        quantityAccepted:Number(row.querySelector('[name=accepted]').value||0),
        quantityRejected:Number(row.querySelector('[name=rejected]').value||0)
      }));
...
export async function openOrderDetail(id){
  ...
  const transitions={
    draft:['requested','cancelled'],requested:['approved','rejected','cancelled'],rejected:['draft','cancelled'],
    approved:['sent','cancelled'],sent:['confirmed','cancelled'],confirmed:['cancelled'],
    partially_received:['received','closed','cancelled'],received:['reconciled','closed'],reconciled:['closed']
  }[order.status]||[];
