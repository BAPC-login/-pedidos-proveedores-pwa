...
function injectLayoutStyles(){
  if($('#companyLogoLayoutStyles'))return;
  const style=document.createElement('style');
  style.id='companyLogoLayoutStyles';
  style.textContent=`
    .master-row{grid-template-columns:minmax(180px,1fr) 125px minmax(135px,.7fr) 102px 82px 92px}
    #companyLogoPreview{display:grid;place-items:center;min-height:180px;border:1px dashed var(--line);border-radius:14px;background:var(--soft)}
    #companyLogoPreview img{max-width:min(280px,90%);max-height:150px;object-fit:contain;background:#fff;padding:6px;border-radius:10px}
    @media(max-width:900px){.master-row{grid-template-columns:minmax(150px,1fr) 115px 125px 95px 76px 88px}}
    @media(max-width:680px){
      .master-row{grid-template-columns:minmax(0,1fr) 90px 76px;gap:7px}
      .master-product{grid-column:1/3}.master-supplier{grid-column:1/4}
      .master-row>[data-master-relation]{grid-column:1/4}
      .master-format{grid-column:1/2}.master-pack{grid-column:2/3}.master-quantity{grid-column:3/4}
      [data-edit-product].master-row{grid-template-columns:minmax(0,1fr) 90px 76px}
      [data-edit-product] .master-product{grid-column:1/4}
    }
  `;
...
function imageToJpeg(file){
  return new Promise((resolve,reject)=>{
    ...
    image.onerror=()=>{URL.revokeObjectURL(source);reject(new Error('El formato no se puede procesar directamente. En iPhone selecciona una imagen JPG, PNG, WebP o una captura de pantalla del logo.'))};image.src=source;
  });
}
...
function enhance(){
  if(state.view!=='settings'||$('#companyLogoQuickButton'))return;
  const company=$('[data-branding-settings]')?.closest('.settings-purpose')||$('[data-branding-settings]')?.parentElement;if(!company)return;
  const button=document.createElement('button');button.id='companyLogoQuickButton';button.className='btn wide-action';button.type='button';button.textContent='Subir logo corporativo directamente';button.dataset.companyLogo='1';company.append(button);
}

export function initializeCompanyLogoUploader(){
  if(initialized)return;initialized=true;injectLayoutStyles();
  document.addEventListener('click',event=>{const target=event.target.closest('[data-company-logo]');if(!target)return;event.preventDefault();openCompanyLogoUploader().catch(error=>toast(error.message,'error'))});
  new MutationObserver(enhance).observe($('#appShell')||document.body,{subtree:true,childList:true});enhance();
}
