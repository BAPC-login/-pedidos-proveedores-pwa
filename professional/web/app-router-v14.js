...
function saveScroll(){const y=scrollTop();sessionStorage.setItem(`pp:scroll:${key(current)}`,String(y));if(history.state?.pp)history.replaceState({...history.state,scrollY:y},'',location.href)}
...
export async function openRoute(view,subview='',options={}){
  ...
  saveScroll();const oldDepth=Number(history.state?.depth||0),same=current.view===view&&current.subview===subview;
  ...
  try{await renderer(route);window.dispatchEvent(new CustomEvent('pedidos:view-rendered',{detail:route}));requestAnimationFrame(()=>window.scrollTo(0,options.restore||options.fromHistory?route.scrollY:0))}finally{document.documentElement.classList.remove('route-loading')}
}
...
export function initialRoute(){const stateRoute=history.state?.pp?history.state:null;if(stateRoute)return stateRoute;const parts=location.hash.replace(/^#\/?/,'').split('/').filter(Boolean);return {view:parts[0]||'dashboard',subview:parts[1]||'',depth:0,scrollY:0}}
