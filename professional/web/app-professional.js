...
function warmMasterOrderV80(){if(masterWarmStarted||!state.token)return;masterWarmStarted=true;const run=()=>{const month=new Date().toISOString().slice(0,7);Promise.allSettled([import('./app-master-order.js'),api('/api/operations-bootstrap-v45',{persist:true,timeout:12000}),api(`/api/budgets?month=${month}`,{persist:true,timeout:10000})]).then(results=>{if(results.slice(1).every(item=>item.status==='rejected'))masterWarmStarted=false}).catch(()=>{masterWarmStarted=false})};if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:1400});else setTimeout(run,120)}
