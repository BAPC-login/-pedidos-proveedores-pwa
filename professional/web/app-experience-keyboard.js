...
function focus(direction){const list=fields(),next=list[list.indexOf(active)+direction];if(next){next.focus({preventScroll:true});next.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(()=>next.select(),60)}else if(direction>0){active?.blur();$('#modalSubmit')?.focus()}}
...
export function initializeQuantityKeyboard(){
  toolbar=document.createElement('div');toolbar.className='quantity-keyboard-toolbar hidden';toolbar.setAttribute('role','toolbar');toolbar.setAttribute('aria-label','Controles del teclado de cantidades');toolbar.innerHTML='<button type="button" data-qty-previous>↑ Anterior</button><button type="button" class="quantity-return" data-qty-next>↵ Enter / siguiente</button><button type="button" data-qty-done>Listo</button>';document.body.append(toolbar);
  ...
  document.addEventListener('focusin',event=>{const input=event.target.closest?.('[data-core-quantity],[data-edit-quantity]');if(!input)return;active=input;input.inputMode='decimal';input.enterKeyHint='next';input.autocomplete='off';toolbar.classList.remove('hidden');document.body.classList.add('quantity-keyboard-open');position()});
