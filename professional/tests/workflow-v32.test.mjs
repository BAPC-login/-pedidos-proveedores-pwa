...
assert.match(index,/2\.0\.0-alpha\.32/);assert.match(index,/\/api\/catalog\/matrix/);assert.match(index,/\/api\/orders\/advanced/);assert.match(index,/\/api\/procurement\/policies/);assert.match(index,/\/api\/learning\/rules/);assert.match(index,/ctx\?\.waitUntil/);
assert.match(combined,/index-v(?:32|33|34|36|38|39|40)\.js/);assert.match(combined,/2026\.08\.(?:05\.33|06\.(?:34|35|36|37|38|39|40|41))/);
assert.match(entry,/initializeOrdersHistoryV32/);assert.match(entry,/initializeCatalogV32/);assert.match(entry,/initializeEnhancementsV32/);assert.match(entry,/initializeInvoiceReviewV36/);assert.match(entry,/app-v39-stability\.js/);
...
assert.match(invoiceEntry,/app-v32-entry\.js/);assert.match(sw,/nuvasto-v(?:32-professional-ux|33-history-documents|34-folio-invoice-visual|36-(?:invoice-review-save-icons|date-only-consistency)|38-multiple-invoices-per-order|39-multi-picker-cache-navigation|40-professional-operations)/);assert.match(sw,/app-v32-catalog\.js/);assert.match(pkg,/2\.0\.0-alpha\.(?:32|33|34|35|36)/);assert.match(pkg,/workflow-v32\.test\.mjs/);

console.log('workflow v32 professional UX compatibility under Nuvasto v40: OK');
