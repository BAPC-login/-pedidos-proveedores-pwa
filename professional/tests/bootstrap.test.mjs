...
assert.ok(bootstrap.includes("from './app-order-core.js'")&&bootstrap.includes('initializeOrderCore()')&&!bootstrap.includes('app-order-core-v15.js'),'canonical bootstrap must own exactly one master-list implementation');
...
assert.ok(/nuvasto-v\d+-offline-data/.test(sw)&&sw.includes("'./app-bootstrap.js'")&&sw.includes("'./app-mobile-runtime.js'")&&sw.includes("'./app-procurement-settings.js'")&&sw.includes("'./app-order-core.js'")&&!sw.includes("'./app-order-core-v15.js'"),'service worker must precache only the canonical critical procurement graph');
