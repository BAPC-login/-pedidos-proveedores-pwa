...
assert.match(worker,/url\.pathname==='\/api\/dev\/qa\/ensure'/,'QA provisioning must be an explicit DEV maintenance route');
assert.match(worker,/runtimeEnvironment\(env\)!=='development'/,'QA provisioning must hard-block production');
assert.match(worker,/DEV_QA_EMAIL='e2e@nuvasto\.dev'/,'QA provisioning must only allow the reserved E2E user');
assert.match(worker,/DEV_QA_SLUG='nuvasto-qa'/,'QA provisioning must stay scoped to the QA organization');
assert.match(devSeed,/\/api\/dev\/qa\/ensure/,'DEV seed must provision or repair QA without manual DB reset');
