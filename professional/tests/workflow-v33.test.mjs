...
assert.match(entry,/initializeDocumentsV33/);assert.match(entry,/documents/);assert.match(combined,/index-v(?:39|40)\.js/);assert.match(combined,/2026\.08\.06\.(?:40|41)/);assert.match(sw,/nuvasto-v(?:39-multi-picker-cache-navigation|40-professional-operations)/);assert.match(sw,/app-v33-documents\.js/);assert.match(pkg,/2\.0\.0-alpha\.36/);assert.match(pkg,/workflow-v33\.test\.mjs/);assert.match(pkg,/index-v33\.js/);assert.match(pkg,/app-v33-documents\.js/);

console.log('workflow v33 history and editable archive compatibility under v40: OK');
