...
assert.match(entry,/initializePolishV34/);assert.match(entry,/initializeInvoiceReviewV36/);assert.match(combined,/index-v(?:39|40)\.js/);assert.match(combined,/2026\.08\.06\.(?:40|41)/);assert.match(sw,/nuvasto-v(?:39-multi-picker-cache-navigation|40-professional-operations)/);assert.match(sw,/app-v34-polish\.js/);assert.match(pkg,/2\.0\.0-alpha\.36/);assert.match(pkg,/workflow-v34\.test\.mjs/);assert.match(pkg,/index-v34\.js/);assert.match(pkg,/app-v34-polish\.js/);

console.log('workflow v34 unique folios, multipart-safe cotejo and visual compatibility under v40: OK');
