...
const canonicalRouter=read('web/app-router.js');
...
assert.ok(professional.includes("from './app-orders.js'")&&professional.includes("from './app-catalog-current.js'")&&professional.includes("from './app-procurement.js'"),'professional bootstrap must use semantic feature owners');
assert.ok(catalogCurrent.includes("registerRouteRenderer('catalog'")&&!catalogCurrent.includes("registerRouteRenderer('suppliers'"),'catalog owner must never register suppliers');
assert.ok(navigation.includes("renderSuppliersWorkspace")&&navigation.includes("registerRouteRenderer('suppliers'"),'navigation must give suppliers to the dedicated current workspace');
assert.ok(canonicalRouter.includes('const renderers=new Map()')&&canonicalRouter.includes("error.code='duplicate_route_owner'")&&canonicalRouter.includes('Ruta duplicada bloqueada'),'canonical router must reject conflicting route owners');
for(const name of ['app-orders.js','app-catalog.js','app-catalog-current.js','app-suppliers.js','app-navigation.js','app-router.js','app-documents.js','app-invoice-review.js','app-enterprise.js','app-procurement.js','app-master-edit.js','app-reception.js','app-operations-dashboard.js'])assert.ok(fs.existsSync(`web/${name}`),`${name} must exist`);
