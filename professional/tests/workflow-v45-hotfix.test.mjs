...
const [files,app,core,index,current,css,sw,combined,pkg,r51,r52,indexHtml,schema]=await Promise.all([
...
assert.match(app,/CLIENT_RELEASE='2026\.08\.10\.60'/);assert.match(app,/verifyClientRelease/);assert.match(app,/seedResponseCache/);assert.match(app,/initializeMasterOrderingV42\(\)/);assert.match(app,/initializeR51UX\(\)/);assert.doesNotMatch(app,/window\.fetch=async/);
...
assert.match(indexHtml,/app-r52-operations\.js/);assert.match(schema,/performance-indexes-r52/);assert.match(schema,/idx_orders_org_status_updated/);
...
assert.match(css,/\.native-share-preparing/);assert.match(sw,/nuvasto-v60-consolidated-runtime/);assert.match(sw,/app-r52-operations\.js/);assert.match(pkg,/workflow-v45-hotfix\.test\.mjs/);

for(const file of ['../web/app-file-actions.js','../web/app.js','../web/app-core.js','../web/app-r51-ux.js','../web/app-r52-operations.js','../web/app-experience-keyboard.js','../worker/src/index-v45.js','../worker/src/index-current.js','../worker/src/schema.js'])execFileSync(process.execPath,['--check',new URL(file,import.meta.url).pathname],{stdio:'inherit'});
console.log('workflow r60 compatibility: r52 operations preserved under consolidated runtime: OK');
