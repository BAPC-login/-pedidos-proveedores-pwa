...
assert.match(mobile,/RELEASE='r95-canonical-safe-area'/,'mobile runtime must retain the proven safe-area implementation');
...
assert.match(sw,/importScripts\('\.\/sw-release\.js'\)/,'installed PWA must use the current generated release');
assert.match(sw,/deleteStaleNuvastoCaches/,'installing a new shell must remove old cache generations');
assert.doesNotMatch(sw,/LEGACY_CACHE_VERSION|PREVIOUS_CACHE_VERSION|PREVIOUS_VERSION/,'safe-area regression coverage must not keep old PWA shells alive');

console.log('safe-area contract: OK · auth, app chrome, content, sheets, search, toasts and bottom navigation stay outside device bars');
