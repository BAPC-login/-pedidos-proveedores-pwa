...
assert.ok(bootstrap.includes("from './app-order-core.js'")&&bootstrap.includes('initializeOrderCore()')&&!bootstrap.includes('app-order-core-v15.js'),'only the semantic master-list owner may be active');
...
assert.ok(orderCore.includes('SERVER_AUTOSAVE_DELAY=10000')&&orderCore.includes('localStorage.setItem(localDraftKey()')&&orderCore.includes('writeLocalEnvelope(payload)'),'every master draft must be protected locally before network autosave');
assert.ok(orderCore.includes("SERVER_DRAFT_KEY='master-order-v15'")&&orderCore.includes('/api/autosave?key=')&&orderCore.includes('Se recuperó tu borrador'),'r69 must recover compatible server autosaves from the prior master implementation');
assert.ok(orderCore.includes("timeout:45000,persist:true")&&orderCore.includes("'Idempotency-Key':payload.idempotencyKey"),'Guardar archivo must use a stable retry-safe idempotency key and an operational timeout');
assert.ok(orderCore.includes("response=await api('/api/order-batches/v2'")&&orderCore.includes('completed=true;clearTimeout(serverAutosaveTimer);markLocalCleared()'),'local recovery data may only be cleared on the successful save branch');
...
assert.ok(!operations.match(/createOrderFileV40[\s\S]{0,260}prepareDraftFoliosV40/),'saving a new draft must not scan or rewrite other draft folios');
...
assert.ok(workerCore.includes("url.pathname==='/api/order-batches/v2'")&&workerCore.includes('createOrderFileV40(request,env,actor)')&&!workerCore.includes("url.pathname==='/api/order-batches/v2'){const actor=await authenticate(request,env);const batch=await withFolioWriteLockV34"),'Guardar archivo must bypass final-folio locks');
assert.ok(workerCore.includes('draftSaveNoFolioLockV69:true')&&workerCore.includes('durableMasterDraftV69:true'),'health must expose durable master draft guarantees');
...
assert.ok(sw.includes("'./app-order-core.js'")&&!sw.includes("'./app-order-core-v15.js'"),'service worker must precache only the canonical master-list owner');
