(function(root){
  ...
  function fresh(){
    ...
    return{version:VERSION,profile:{...DEFAULT_PROFILE},settings:{theme:'light'},catalog:{categories,providers},items,draft:{},currentOrderFolio:null};
  }
  ...
  function migrate(raw){
    ...
    const categoryFor=item=>{
      let name=String(item.category||'').trim();
      if(!name||providers.some(provider=>core().normalizeText(provider.name)===core().normalizeText(name)))name=core().inferCategory(item.description);
      return categories.find(entry=>core().normalizeText(entry.name)===core().normalizeText(name))||categories.find(entry=>entry.name==='OTROS')||categories[0];
    };
    ...
    const items=raw.items.map((item,index)=>({
      id:String(item.id||`item-${Date.now()}-${index}`),description:String(item.description||'').trim(),providerId:item.providerId||providerFor(item)?.id||'',categoryId:item.categoryId||categoryFor(item)?.id||'',unit:UNITS.includes(item.unit)?item.unit:'UNIDAD',enabled:item.enabled!==false,sortOrder:Number(item.sortOrder??index)
    ...
    const profile={...DEFAULT_PROFILE,...raw.profile,folioPrefix:raw.profile?.folioPrefix||core().sanitizePrefix(raw.profile?.location||'MDR')};
    return{...raw,version:VERSION,profile,settings:{theme:'light',...raw.settings},catalog:{categories,providers},items,draft,currentOrderFolio:raw.currentOrderFolio||raw.currentFolio||null};
  }
  ...
  async function init(){
    await root.PedidosDB.open();
    await root.PedidosDB.migrateLegacyAssets(state,core().normalizeText);
    await root.PedidosDB.migrateLegacyOrders(state);
    if(state.profile){delete state.profile.logo;delete state.profile.logo2}
    delete state.providerLogos;delete state.providerLogoSizes;state.catalog.providers.forEach(entry=>delete entry.logo);persist();root.appState=state;return state;
  }

  root.PedidosState={KEY,VERSION,UNITS,DEFAULT_CATEGORIES,DEFAULT_PROFILE,get value(){return state},fresh,migrate,persist,setState,provider,category,item,enabledProviders,enabledCategories,draftRows,init};
  root.appState=state;
})(globalThis);
