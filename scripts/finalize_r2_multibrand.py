...
# Invoice history is linked to one or more locals and exposes its R2 object.
p = Path("professional/worker/src/api/documents.js")
t = p.read_text(encoding="utf-8")
safe = "function safeJson(value, fallback = null) {\n  try { return JSON.parse(value || ''); } catch { return fallback; }\n}\n"
helper = """
...
if helper.strip() not in t:
    t = replace_once(t, safe, safe + helper, "documents location helper")
old = """    SELECT i.*, s.name AS supplier_name
...
new = """    SELECT i.*, s.name AS supplier_name,
...
t = replace_once(t, old, new, "invoice listing query")
t = replace_once(
    t,
...
t = replace_once(
  ...
  const invoiceId = uuid();""",
    "invoice locations initialization",
)
t = replace_once(
    t,
    """  const orderIds = [...new Set((Array.isArray(body.orderIds) ? body.orderIds : []).map(String).filter(Boolean))];
...
t = replace_once(
    ...
    "invoice snapshot local",
)
p.write_text(t, encoding="utf-8")


# Dashboard metrics and history are scoped by the actor's allowed locals.
p = Path("professional/worker/src/api/catalog.js")
t = p.read_text(encoding="utf-8")
pattern = re.compile(r"export async function dashboard\(env, actor\) \{.*?\n\}\n\nexport async function listLocations", re.S)
replacement = """export async function dashboard(env, actor) {
...
if count != 1:
    raise SystemExit(f"Could not replace dashboard: {count}")
t = replace_once(
    t,
    ...
    "platform owner location limit",
)
p.write_text(t, encoding="utf-8")


# Current workspace can create additional locals.
p = Path("professional/web/app-actions.js")
t = p.read_text(encoding="utf-8")
marker = "function openBrand(){"
location = """function openLocation(){
...
"""
t = replace_once(t, marker, location + marker, "open location UI")
t = replace_once(
    t,
...
# Views: historical files, local user scopes and owner brand manager.
p = Path("professional/web/app-views.js")
t = p.read_text(encoding="utf-8")
old_metrics = "${metric('Pedidos 30 días',m.orders30d,'Órdenes creadas')}${metric('Pendientes',m.pendingOrders,'Requieren gestión')}${metric('Compras 30 días',money(m.spend30d),'Facturas registradas')}${metric('Proveedores',m.suppliers,'Activos')}${metric('Productos',m.products,'En catálogo')}${metric('Diferencias',m.openIssues,'Sin resolver')}"
t = replace_once(t, old_metrics, old_metrics + "${metric('Documentos R2',m.archivedDocuments||0,'Históricos archivados')}", "dashboard R2 metric")
status = "const statusLabel=value=>({draft:'Borrador',requested:'Solicitado',approved:'Aprobado',rejected:'Rechazado',sent:'Enviado',confirmed:'Confirmado',partially_received:'Recepción parcial',received:'Recibido',reconciled:'Conciliado',closed:'Cerrado',cancelled:'Anulado'}[value]||value);\n"
view_helper = """
...
"""
t = replace_once(t, status, status + view_helper, "document opener")
invoice_pattern = re.compile(r"function invoiceTable\(invoices\)\{.*?\n\}", re.S)
invoice_replacement = """function invoiceTable(invoices){
  ...
  return `<table class="data-table"><thead><tr><th>Documento</th><th>Proveedor</th><th>Local</th><th>Fecha</th><th>Estado</th><th>Total</th><th>Archivo</th></tr></thead><tbody>${invoices.map(i=>`<tr><td><strong>${esc(i.invoiceNumber)}</strong><br><small>Tipo ${esc(i.documentType)}</small></td><td>${esc(i.supplierName)}</td><td>${esc((i.locationNames||[]).join(', ')||'—')}</td><td>${date(i.invoiceDate)}</td><td><span class="status ${esc(i.status)}">${esc(i.status)}</span></td><td><strong>${money(i.grossTotal)}</strong></td><td>${i.pdfKey?`<button class="btn small" data-file-key="${esc(i.pdfKey)}">Abrir original</button>`:'—'}</td></tr>`).join('')}</tbody></table>`;
}"""
t, count = invoice_pattern.subn(invoice_replacement, t, count=1)
if count != 1:
    raise SystemExit("Could not replace invoice table")
old_search = "$('#invoiceSearch').addEventListener('input',()=>{const q=$('#invoiceSearch').value.toLowerCase();$('#invoiceContainer').innerHTML=invoiceTable(state.cache.invoices.filter(i=>`${i.invoiceNumber} ${i.supplierName}`.toLowerCase().includes(q)))});bindDynamic();"
new_search = "$('#invoiceSearch').addEventListener('input',()=>{const q=$('#invoiceSearch').value.toLowerCase();$('#invoiceContainer').innerHTML=invoiceTable(state.cache.invoices.filter(i=>`${i.invoiceNumber} ${i.supplierName} ${(i.locationNames||[]).join(' ')}`.toLowerCase().includes(q)));bindStoredDocuments()});bindDynamic();bindStoredDocuments();"
t = replace_once(t, old_search, new_search, "invoice search binding")
